...
from task.domain.model import User
...
def task_service_integration(
    task_repo_real: SQLAlchemyTaskRepository, user_repo_real: SQLAlchemyUserRepository
) -> TaskService:
...
class TestUserServiceIntegration:
    ...
    async def test_get_non_existent_user_raises_error(
        self, user_service_integration: UserService
    ):
...
class TestTaskServiceIntegration:
    ...
    async def test_create_task(
        self, task_service_integration: TaskService, registered_user: User
    ):
    ...
    async def test_create_task_for_non_existent_user(
        self, task_service_integration: TaskService
    ):
    ...
    async def test_mark_done_and_list_tasks(
        self, task_service_integration: TaskService, registered_user: User
    ):
    ...
    async def test_delete_task(
        ...
        assert registered_user.id is not None
        dto = TaskCreateRawData(
            user_id=registered_user.id, text="Удалить тестовый файл"
        )
    ...
    async def test_operation_on_non_existent_task_raises_error(
        self, task_service_integration: TaskService
    ):
